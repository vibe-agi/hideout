...
1. Check free space and native aarch64 builder identity. Use the direct
   libguestfs backend with KVM when `/dev/kvm` is usable and deterministic TCG
   fallback otherwise; record the backend, backend settings, and acceleration
   mode. A direct local build records `native-unpinned`; only retained CI
   provenance may bind the locked builder image declaration for promotion
   review.
...
6. Bind each gate to the clean image build commit and exact runtime digest, and
   independently bind the enclosing product evidence to the later clean,
   verified Hideout package commit that embeds the promoted catalog. Both
   identities are mandatory; equality between them is neither expected nor
   accepted as a substitute for either binding.
