...
image-build provenance, clean verified package identity, or either real gate is
missing.

After both real gates, execute `scripts/test-runtime-quickstart.sh`. It reruns
the local mechanics and documentation checks, validates artifact digests and
all six required real proof IDs, requires distinct Gate 2/Gate 3 environment
identities over one exact runtime build, and writes the exact 12-scenario
receipt to `dist/runtime/evidence/031-quickstart/quickstart.out`. The receipt is
completion evidence, not a substitute for either retained real gate manifest.
