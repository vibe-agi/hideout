...
1. **Given** the selected runtime, **When** real Gate 2 and Gate 3 execute in independently created disposable environments, **Then** each proof carries its own valid environment identity while both bind to the same runtime revision, image digest, architecture, and candidate source state.
...
- **Runtime Evidence**: Gate and product-hardening records binding artifact, environment, network path, tool checks, and candidate source state.
...
- **SC-008**: Real Gate 2 and Gate 3 evidence each report a valid non-empty,
  ...
  artifact digest, architecture, and candidate source state. Reusing one
