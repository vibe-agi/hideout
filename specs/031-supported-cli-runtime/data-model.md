...
environments. Equality is required for immutable runtime artifact and image
build fields, not for the two mutable environment identities. The enclosing
product-evidence manifest separately binds the clean Hideout package candidate
that embeds the promoted catalog; this later package commit is not the image
build commit.
