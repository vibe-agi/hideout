...
- `--rm` owns a per-run dedicated record marked disposable; its proved
  teardown removes the record, and an unproved teardown retains it in error
  state for `hideout clean`. No reusable record survives a successful run.
  `--ephemeral` keeps identity state session-local while resolving the same
  reusable environment as the corresponding normal run.
