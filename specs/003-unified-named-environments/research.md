...
model; `--rm` owns a per-run dedicated record marked disposable that its
proved teardown removes, so no reusable record survives a successful run.
`--ephemeral` is a session-identity modifier and resolves the same reusable
environment as a normal run.
