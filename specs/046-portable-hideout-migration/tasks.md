...
- [ ] T117 CRITICAL: Replace the migration gate's permissive test-name regexes with an explicit fail-on-drift inventory covering CLI, Manager API, daemon recovery, TUI, WebUI, redaction, and targeted races, then add the installed package-candidate `scripts/gates/migration-lima.sh` evidence required before any full-state claim per Constitution IV and FR-048 (repository-wide active-source discovery and the 229-test/19-package inventory are complete; exact clean-candidate execution pending)
