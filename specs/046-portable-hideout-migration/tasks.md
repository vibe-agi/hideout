...
- [ ] T117 CRITICAL: Replace the migration gate's permissive test-name regexes with an explicit fail-on-drift inventory covering CLI, Manager API, daemon recovery, TUI, WebUI, redaction, and targeted races, then add the installed package-candidate `scripts/gates/migration-lima.sh` evidence required before any full-state claim per Constitution IV and FR-048 (inventory and gate implementation complete; exact clean-candidate execution pending)
...
- [x] T125 Isolate release verification from ambient Go module-write policy by pinning Gate 0 and migration tests to read-only module mode and proving a representative run leaves the tidy module ledger unchanged
