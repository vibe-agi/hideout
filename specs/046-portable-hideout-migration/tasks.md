...
- [x] T141 Close the root-runtime-to-private-target-view refinement: carry only authenticated imported disk bindings into each Portal session view, re-prove the exact mount/type/read-write/alias facts before projecting the destination and original path, exclude the broad `/mnt` tree, and model disk-view readiness as a separate prerequisite of target start per FR-053
