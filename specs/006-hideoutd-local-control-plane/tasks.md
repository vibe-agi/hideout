...
- [X] T010 [P] [US1] Unit: Manager parity drift-guard — the daemon-served Manager route set equals the complete production `manager.ManagerRoutes()` inventory; one plan/apply through the daemon matches embedded, in `internal/daemon/server_test.go` (FR-005, SC-002)
