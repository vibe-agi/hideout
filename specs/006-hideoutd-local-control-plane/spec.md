...
3. **Given** the TUI or WebUI smoke surface connected to the daemon, **When** environment or run state changes, **Then** the surface reflects the change from events alone (its existing panels; no console redesign).
...
- **FR-016**: The daemon's own lifecycle/observability endpoints (status/inventory and event subscription) are a separate surface outside the Manager subrouter and MUST be inventoried explicitly. They MUST add no Manager operation class, raw profile write, or host execution, and are subject to the same authentication and redaction rules.
...
- **FR-009**: The existing TUI and WebUI smoke surfaces MUST be able to consume daemon event streams for their current panels (live refresh instead of polling) without a console redesign; TUI scope stays lightweight parity.
...
- **SC-003**: With the daemon present, a connected surface reflects 100% of tested operation/environment state changes from streamed events alone — zero polling reads of overview resources during the steady-state window.
...
- **Daemon-first is the declared steady state**: The TUI/WebUI experience contract already states surfaces consume daemon event streams for live state instead of polling, and that the daemon improves freshness without changing TUI scope. 006 delivers the substrate plus stream consumption by the existing smoke surfaces; a fuller operations console is a follow-on (007 candidate), not part of 006.
