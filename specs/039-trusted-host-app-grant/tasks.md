...
- [X] T011 [US1] Implement the grant command in `internal/app/operator_access.go`: derive `workspaceID` from the project dir (same functions a run uses), read the exact Core-derived `qualifiedAppRef` + `bindingDigest` from the request hint recorded by the prior refused trusted run for this `(workspace, command)`, and write the grant under the profile mutation lock; require host-app-mode trusted (else name `host-app-mode trusted`) and fail closed when the matching request is absent or malformed. Wire dispatch in `internal/app/app.go` and add the usage line.
