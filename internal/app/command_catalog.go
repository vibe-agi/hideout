	...
	return commandCatalog{entries: []commandCatalogEntry{
		...
		},
		{
			spec: commandSpec{
				...
				Flags: []commandFlag{
					...
					{Name: "--incarnation", Value: "<id>", Help: "select one exact VM instance ID"},
				...
				Effects:       []string{"Queries retained metadata for the selected run or exact VM instance; it never reads file contents, environment variables, keystrokes, or full terminal output."},
				Safety:        readOnlySafety,
		...
		},
		{
			spec: commandSpec{
				...
				Effects:       []string{"Reads Hideout's current verified state. Configuration and lifecycle changes still require review and confirmation."},
		...
		},
		{
			spec: commandSpec{
				...
				Syntax:  []string{"hideout ui [--no-open] [--print-url]"},
				Flags: []commandFlag{
					{Name: "--no-open", Help: "do not launch a browser"},
					{Name: "--print-url", Help: "print the browser URL and exit"},
				},
				Examples:      []string{"hideout ui", "hideout ui --no-open", "hideout ui --print-url"},
				Prerequisites: installed,
				Effects:       []string{"Ensures hideoutd is running, opens its live local-only console, then exits without stopping the daemon."},
				Safety:        []string{"The listener must remain loopback-only; do not share its bearer URL or token."},
				Recovery:      []string{"Close the browser; rerun the command for a URL carrying the current rotating credential."},
		...
		},
		{
			spec: commandSpec{
				...
				Prerequisites: daemonReady,
				Effects:       []string{"Set, rotate, and delete change one named secret version; status and list never reveal values."},
		...
		},
		{
			spec: commandSpec{
				...
				Effects:       []string{"Deletes the selected environment runtime, activity recorded for that exact VM instance, and retained local data."},
