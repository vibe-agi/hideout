...
func TestEvaluateRuntimeProofRequiresExactCleanRealBinding(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "dirty candidate", mutate: func(p *ProofEntry) { p.Runtime.CandidateDirty = true }, want: EvalRuntimeMismatch},
		{name: "wrong commit", mutate: func(p *ProofEntry) { p.Runtime.CandidateCommit = "abcdef012345" }, want: EvalRuntimeMismatch},
	}
...
func TestEvaluateReleaseRuntimeProofRequiresTrustedPackageAndArtifactDigest(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "stale manifest with forged notes", mutate: func(m *Manifest, _ *EvaluationOptions) {
			m.Commit = "unrelated"
			m.Proofs[0].Notes = []string{"commit=" + expected.CandidateCommit, "package=hideout@pkg-0123456789ab"}
		}, want: EvalStale},
...
func runtimeExpectationFixture() RuntimeExpectation {
	...
	return RuntimeExpectation{
		...
		CandidateCommit: binding.CandidateCommit, RequireClean: true,
	}
}
...
func runtimeManifestWithProofs(expected RuntimeExpectation, proofs ...ProofEntry) Manifest {
	manifest := evalManifestWithProofs(proofs...)
	manifest.Commit = expected.CandidateCommit
	manifest.PackageIdentity = &PackageIdentity{Name: "hideout", Version: expected.CandidateCommit}
	return manifest
}
...
func runtimeEvaluationOptions(requirements []ProofRequirement, expected RuntimeExpectation) EvaluationOptions {
	return EvaluationOptions{
		...
		ExpectedCommit:  expected.CandidateCommit,
		ExpectedPackage: &PackageIdentity{Name: "hideout", Version: expected.CandidateCommit},
