	...
	for _, req := range reqs {
		...
		if result.Status == EvalSatisfied && req.RuntimePolicy == RuntimePolicyExactReal {
			...
			if runtimeAnchor == nil {
				copy := *binding
				runtimeAnchor = &copy
			} else if !runtimeAnchor.SameArtifactCandidate(*binding) {
...
func runtimeProofIsStale(identity proofFreshnessIdentity, opts EvaluationOptions) bool {
	...
	candidateCommit := opts.ExpectedRuntime.CandidateCommit
	if !IsCanonicalCommit(identity.commit) || identity.commit != candidateCommit {
		return true
	}
	if opts.ExpectedPackage == nil || opts.ExpectedPackage.ValidateCandidateCommit(candidateCommit) != nil {
		return true
	}
	if identity.packageIdentity == nil || identity.packageIdentity.ValidateCandidateCommit(candidateCommit) != nil {
		return true
	}
