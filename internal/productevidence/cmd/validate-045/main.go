...
func main() {
	...
	if flag.NArg() != 1 || !productevidence.IsCanonicalCommit(strings.TrimSpace(*commit)) || strings.TrimSpace(*packagePath) == "" {
		fmt.Fprintln(os.Stderr, "usage: validate-045 --commit <full-commit> --package-identity <json> <manifest>")
		os.Exit(2)
	}
	...
	err = productevidence.Require045ReleaseComplete(manifest, productevidence.EvaluationOptions{
	...
	fmt.Println("validate-045: release-candidate passed")
}
...
func fail(err error) {
	fmt.Fprintln(os.Stderr, "validate-045:", err)
	os.Exit(1)
}
