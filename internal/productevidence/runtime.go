...
// RuntimeBinding ties a proof to one real guest artifact and the clean source
// state that built that artifact. Package identity is recorded separately on
// the enclosing evidence manifest because the final package embeds the
// promoted artifact digest and is therefore built from a later commit.
...
// RuntimeExpectation is anchored in the packaged catalog. EnvironmentID is
...
func (e RuntimeExpectation) Validate() error {
	for name, value := range map[string]string{
		...
		"hostArch": e.HostArch, "guestArch": e.GuestArch, "buildCommit": e.BuildCommit,
	} {
	...
	if !IsCanonicalCommit(e.BuildCommit) {
		return errors.New("runtime expectation buildCommit must be a canonical 12-40 character lowercase hex commit")
	}
	if !e.RequireClean {
		return errors.New("runtime expectation must require a clean image build source state")
	}
	return nil
}
...
func (b RuntimeBinding) Validate() error {
	...
	for name, value := range map[string]string{
		...
		"hostArch": b.HostArch, "guestArch": b.GuestArch, "buildCommit": b.BuildCommit,
	} {
		if strings.TrimSpace(value) == "" || len(value) > 128 || audit.RedactString(value) != value {
	...
	if !IsCanonicalCommit(b.BuildCommit) {
		return errors.New("runtime evidence buildCommit must be a canonical 12-40 character lowercase hex commit")
	}
...
func (b RuntimeBinding) Sanitized() RuntimeBinding {
	...
	b.BuildCommit = audit.RedactString(b.BuildCommit)
	return b
}
...
func (b RuntimeBinding) Matches(expected RuntimeExpectation) error {
	...
	checks := []struct{ name, got, want string }{
		...
		{"buildCommit", b.BuildCommit, expected.BuildCommit},
	}
	...
	if expected.RequireClean && b.BuildDirty {
		return errors.New("runtime evidence image build source state is dirty")
	}
	return nil
}

func (b RuntimeBinding) SameArtifactBuild(other RuntimeBinding) bool {
	return b.Schema == other.Schema &&
		...
		b.BuildCommit == other.BuildCommit &&
		b.BuildDirty == other.BuildDirty
}
