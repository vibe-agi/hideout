...
func TestStartLocalServerServesUIAndAPI(t *testing.T) {
	...
	for _, want := range []string{
		...
		`Showing newest`,
		`profileEnvForm`,
		`profileEnvPayloadFromForm`,
		`profile/env/`,
		`data-profile-env-action="plan"`,
		`data-profile-env-action="apply"`,
		`set public`,
		`only used by set public`,
		`hideout audit show --session`,
		`hideout cleanup --session`,
