...
func main() {
	...
	flag.Parse()
	if flag.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "build-workspace-portal: positional arguments are not supported")
		os.Exit(2)
	}
	if err := helperbin.BuildLinuxWorkspacePortal(opts); err != nil {
		fmt.Fprintln(os.Stderr, "build-workspace-portal:", err)
		os.Exit(1)
	}
}
