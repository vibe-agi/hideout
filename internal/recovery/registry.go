...
const (
	...
	CodeSetupDaemonUnavailable        = "setup.daemon.unavailable"
	CodeSetupProfileRepairRequired    = "setup.profile.repair-required"
	CodeSetupProfileBlocked           = "setup.profile.blocked"
	CodeSetupPlanStale                = "setup.plan.stale"
...
var registry = []Code{
	...
	{Code: CodeSetupDaemonUnavailable, Subsystem: "setup", Severity: "error", Reason: "the authenticated local control plane could not become ready", Hint: "inspect daemon status and retry setup; no embedded writer is used", NextActions: []string{"hideout daemon status", "hideout doctor --feature daemon --level deep"}, DocsRefs: []string{"docs/manager-control-plane.md", "docs/first-run-alpha.md"}},
	{Code: CodeSetupProfileRepairRequired, Subsystem: "setup", Severity: "warning", Reason: "the default profile exists but its identity or metadata is incomplete", Hint: "review the typed repair plan; setup will not normalize existing state", NextActions: []string{"hideout doctor --fix --dry-run --profile default --backend lima"}, DocsRefs: []string{"docs/first-run-alpha.md"}},
	{Code: CodeSetupProfileBlocked, Subsystem: "setup", Severity: "error", Reason: "the existing default profile is malformed, unsafe, or cannot be proved readable", Hint: "inspect the profile and store placement before any explicit recovery", NextActions: []string{"hideout doctor --profile default --backend lima"}, DocsRefs: []string{"docs/first-run-alpha.md", "docs/threat-model.md"}},
	{Code: CodeSetupPlanStale, Subsystem: "setup", Severity: "warning", Reason: "profile or runtime state changed after the reviewed setup plan", Hint: "rerun setup and review the current plan; no stale plan was applied", NextActions: []string{"hideout setup"}, DocsRefs: []string{"docs/first-run-alpha.md", "docs/manager-control-plane.md"}},
	...
	{Code: CodePackagePrerequisiteMissing, Subsystem: "package", Severity: "warning", Reason: "a required package-owned helper is missing, damaged, or invalid", Hint: "verify or reinstall the exact Hideout package; explicit development overrides must validate and never fall through", NextActions: []string{"hideout package verify <install-prefix>"}, DocsRefs: []string{"docs/distribution-bootstrap.md"}},
	...
	{Code: CodeSessionCleanupFailed, Subsystem: "session", Severity: "error", Reason: "a run session could not prove complete cleanup of its own authority", Hint: "inspect session status; an explicit environment stop can prove guest authority absent and recover stale ownership", NextActions: []string{"hideout doctor --level deep", "hideout env list", "hideout stop <environment>"}, DocsRefs: []string{"docs/privacy-run-design.md", "docs/threat-model.md"}},
	...
	{Code: CodeProjectionModeTrustedDenied, Subsystem: "projection", Severity: "warning", Reason: "trusted host-app mode requires an explicit per-project grant", Hint: "grant it on the host with: hideout allow host-app <command>, or use the default safe mode", NextActions: []string{"hideout allow host-app <command>", "hideout profile host-app-mode <profile> trusted"}, DocsRefs: []string{"docs/host-capability-projection.md"}},
