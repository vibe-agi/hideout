...
func TestEmbeddedUnpromotedCatalogIsInspectableButNotSelectable(t *testing.T) {
	catalog, err := LoadEmbedded()
	if err != nil {
		t.Fatal(err)
	}
	if catalog.CatalogRelease != UnpromotedRelease || len(catalog.Families) != 0 {
		t.Fatalf("embedded unpromoted catalog=%+v", catalog)
	}
	if _, err := catalog.Resolve(Selection{Family: "developer-standard", HostOS: "darwin", HostArch: "arm64"}); err == nil {
		t.Fatal("unpromoted catalog should not resolve")
	}
}
