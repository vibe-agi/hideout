...
func BuiltinMatrix() Matrix {
	return Matrix{
		...
		Entries: []Entry{
			...
			{Area: "helper", Subject: "helper/linux-observer", Level: LevelSupported, Reason: "fixed packaged guest workload observer with digest-bound CO-RE programs", Guidance: "runtime coverage is authoritative; require the exact 045 real-Lima observation gate", RequiredGates: []string{"gate0", "gate2-lima"}, Evidence: []string{"package-smoke", "workload-observation-lima"}},
			...
			{Area: "feature", Subject: "feature/workload-observation-process", Level: LevelGateRequired, Reason: "exact command and descendant attribution requires a proved cgroup-v2 boundary and healthy observer", Guidance: "require an Available process interval for the exact run and the 045 real-Lima gate", RequiredGates: []string{"gate2-lima"}, Evidence: []string{"workload-observation-lima"}},
			{Area: "feature", Subject: "feature/workload-observation-file", Level: LevelDegraded, Reason: "the current reference provider records attributed file metadata but reports Partial completeness", Guidance: "inspect the run's file coverage interval and reason; never infer no access from an empty result", RequiredGates: []string{"gate2-lima"}, Evidence: []string{"workload-observation-lima"}},
			{Area: "feature", Subject: "feature/workload-observation-network", Level: LevelDegraded, Reason: "the current reference provider records process-to-IP/port evidence but reports Partial route attribution", Guidance: "inspect the run's network coverage and attribution grade before drawing a route conclusion", RequiredGates: []string{"gate2-lima"}, Evidence: []string{"workload-observation-lima"}},
			{Area: "feature", Subject: "feature/workload-observation-dns", Level: LevelDegraded, Reason: "plaintext DNS metadata is observable, while encrypted, cached, or ambiguous name attribution remains Partial or unknown", Guidance: "inspect DNS coverage and correlation reason; Hideout does not decrypt encrypted DNS", RequiredGates: []string{"gate2-lima"}, Evidence: []string{"workload-observation-lima"}},
		...
		NonClaims: []NonClaim{
			{ID: "guest-root-containment", Summary: "Hideout does not claim containment after the target obtains guest root.", AppliesTo: []string{"feature/guest-privilege-separation", "feature/workload-observation-process", "feature/workload-observation-file", "feature/workload-observation-network", "feature/workload-observation-dns", "backend/lima"}, Guidance: "guest root can tamper with the workload boundary or observer; degrade affected coverage and do not claim containment"},
