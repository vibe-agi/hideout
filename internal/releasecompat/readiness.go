...
func BuildReadiness(opts ReadinessOptions) (Readiness, error) {
	...
	if mode == "release-candidate" {
		commit = ""
		if opts.Runtime != nil && productevidence.IsCanonicalCommit(opts.Runtime.CandidateCommit) {
			commit = opts.Runtime.CandidateCommit
		}
	}
	if commit == "" {
		commit = "unknown"
	...
	if opts.Runtime != nil && len(ready.Gates) == 2 && ready.Gates[0].Runtime != nil && ready.Gates[1].Runtime != nil {
		if !ready.Gates[0].Runtime.SameArtifactCandidate(*ready.Gates[1].Runtime) {
			ready.Gates[1].Status = "failed"
			ready.Gates[1].Summary = "Gate 3 runtime artifact and candidate binding does not match Gate 2"
	...
	gatesValid := validateReleaseGateRows(ready.Gates, commit) == nil
	ready.ReleaseReady = opts.LocalPassed && releaseTrustAnchorsValid(opts.Runtime, opts.Package) && commandsValid && gatesValid
...
func productHardeningCommand(paths []string, commit string, runtimeExpectation *productevidence.RuntimeExpectation, expectedPackage *productevidence.PackageIdentity, releaseCandidate bool) CommandResult {
	if releaseCandidate {
		if !releaseTrustAnchorsValid(runtimeExpectation, expectedPackage) {
		...
		commit = runtimeExpectation.CandidateCommit
	}
	if len(paths) == 0 {
		if releaseCandidate {
...
func releaseTrustAnchorsValid(runtimeExpectation *productevidence.RuntimeExpectation, expectedPackage *productevidence.PackageIdentity) bool {
	if runtimeExpectation == nil || runtimeExpectation.Validate() != nil || expectedPackage == nil {
		return false
	}
	return expectedPackage.ValidateCandidateCommit(runtimeExpectation.CandidateCommit) == nil
}
...
func ValidateReadiness(readiness Readiness) error {
	...
	if err := validateReleaseGateRows(readiness.Gates, readiness.Commit); err != nil {
...
func validateReleaseGateRows(gates []GateResult, candidateCommit string) error {
	expected := map[string]struct{}{
		"gate2-lima":         {},
		"gate3-hidden-proxy": {},
	}
	...
	for _, gate := range gates {
		...
		if gate.Runtime.CandidateDirty || gate.Runtime.CandidateCommit != candidateCommit {
			return fmt.Errorf("gate %q does not bind the clean readiness candidate commit", gate.ID)
		}
		if runtimeAnchor == nil {
			copy := *gate.Runtime
			runtimeAnchor = &copy
		} else if !runtimeAnchor.SameArtifactCandidate(*gate.Runtime) {
			return fmt.Errorf("gate %q runtime artifact and candidate binding does not match the other required gate", gate.ID)
		}
	}
	return nil
}
