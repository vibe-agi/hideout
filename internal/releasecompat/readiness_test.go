...
func TestReleaseCandidateRuntimeEvidenceRequiresExactCleanCrossGateBinding(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "dirty gate", product: true, mutateGate2: func(b *productevidence.RuntimeBinding) { b.CandidateDirty = true }},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.product {
				...
				writeRuntimeProductEvidence(t, product, expected.CandidateCommit, proofBinding)
				if tc.removePrior {
...
func TestRuntimeReadinessRejectsAbsentStaleNativeAndFailedEvidence(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			product := filepath.Join(dir, "runtime-product.json")
			writeRuntimeProductEvidence(t, product, expected.CandidateCommit, binding)
			...
			ready, err := BuildReadiness(ReadinessOptions{
				Mode: "release-candidate", Commit: expected.CandidateCommit, LocalPassed: true,
...
func TestReleaseCandidateRequiresOneCanonicalRuntimePackageCommit(t *testing.T) {
	expected := runtimeExpectationFixture()
	binding := runtimeBindingFixture()
	for _, packageCommit := range []string{"pkg-0123456789ab", "abcdef012345"} {
		t.Run(packageCommit, func(t *testing.T) {
			dir := t.TempDir()
			gate2 := filepath.Join(dir, "gate2.json")
			gate3 := filepath.Join(dir, "gate3.json")
			product := filepath.Join(dir, "runtime-product.json")
			...
			writeRuntimeProductEvidence(t, product, expected.CandidateCommit, binding)
			packageIdentity := packageIdentityFixture()
			packageIdentity.Version = packageCommit
			ready, err := BuildReadiness(ReadinessOptions{
				Mode: "release-candidate", LocalPassed: true, Gate2Evidence: gate2, Gate3Evidence: gate3,
				ProductEvidence: []string{product}, Runtime: &expected, Package: &packageIdentity,
			})
			...
			if ready.ReleaseReady {
				t.Fatalf("runtime/package candidate mismatch produced release readiness: %+v", ready)
			}
			if ready.Commit != expected.CandidateCommit {
				t.Fatalf("readiness commit=%q want runtime candidate %q", ready.Commit, expected.CandidateCommit)
			}
		})
	}
}
...
func TestValidateReadinessRequiresExactReleaseRows(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "candidate mismatch", mutate: func(r *Readiness) { r.Gates[1].Runtime.CandidateCommit = "abcdef012345" }},
...
func validReleaseReadinessFixture() Readiness {
	...
	return Readiness{
		...
		Mode: "release-candidate", EvidenceClass: "real-gate", ReleaseReady: true, Status: "passed", Commit: gate2Runtime.CandidateCommit,
...
func runtimeBindingFixture() productevidence.RuntimeBinding {
	return productevidence.RuntimeBinding{
		...
		CandidateCommit: "0123456789ab", CandidateDirty: false,
	}
}
...
func runtimeExpectationFixture() productevidence.RuntimeExpectation {
	...
	return productevidence.RuntimeExpectation{
		...
		CandidateCommit: b.CandidateCommit, RequireClean: true,
	}
}

func packageIdentityFixture() productevidence.PackageIdentity {
	return productevidence.PackageIdentity{Name: "hideout", Version: "0123456789ab"}
}
