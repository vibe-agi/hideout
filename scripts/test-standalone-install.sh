...
if [ -z "$package" ]; then
  candidate="$ROOT/.hideout-release-evidence/044-alpha3-exact-candidate-b513945/candidate-assets/hideout-v0.1.0-alpha.3-darwin-arm64.tar.gz"
  if [ -f "$candidate" ]; then
    package="$candidate"
  fi
fi
...
version="$(jq -er '.current.version' releases/current.json)"
"$tmp/prefix/bin/hideout" version >"$tmp/version.out"
grep -Fx "hideout $version" "$tmp/version.out" >/dev/null
"$tmp/prefix/bin/hideout" package verify "$tmp/prefix" >/dev/null
test ! -e "$tmp/store/profiles/default/profile.json"
grep -F "Hideout $version installed at $tmp/prefix/bin/hideout" "$tmp/install.out" >/dev/null
