...
runtime_evidence_markers() {
  ...
  local commit="${HIDEOUT_RUNTIME_CANDIDATE_COMMIT:-}"
  local dirty="${HIDEOUT_RUNTIME_CANDIDATE_DIRTY:-}"
  ...
  jq -r \
    ...
    '"runtime_family=" + .provenance.family,
     ...
     "runtime_candidate_commit=" + $commit,
     "runtime_candidate_dirty=" + ($dirty|tostring)' \
    "$receipt"
}
...
emit_gate_result() {
  ...
  for value in \
    ...
    "${HIDEOUT_RUNTIME_EVIDENCE_CANDIDATE_COMMIT:-}"; do
  ...
  if [ "$runtime_declared" -eq 1 ]; then
    ...
    local runtime_commit="${HIDEOUT_RUNTIME_EVIDENCE_CANDIDATE_COMMIT:-}"
    local runtime_dirty="${HIDEOUT_RUNTIME_EVIDENCE_CANDIDATE_DIRTY:-}"
    ...
    case "$runtime_dirty" in
      true | false) ;;
      *) echo "emit_gate_result: runtime candidate dirty state must be true or false" >&2; return 2 ;;
    esac
    runtime_json="$(jq -n \
      ...
      --arg candidateCommit "$runtime_commit" \
      --argjson candidateDirty "$runtime_dirty" \
      '{schema:$schema,family:$family,revision:$revision,artifactSHA256:$artifactSHA256,
        ...
        candidateCommit:$candidateCommit,candidateDirty:$candidateDirty}')"
  fi
