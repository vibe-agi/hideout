...
# pre-render deltas are preservation entries for newly packaged helpers and the
# strict code-signing verification required by the entitled Darwin executor.
...
homebrew_formula_matches_published_snapshot() {
  ...
  local host_preservation_comment='  # The Darwin VZ executor is preserved to keep its entitlement-bearing signature.'
  local host_preservation_line='             "bin/hideout-migration-vz-adopt-darwin-arm64",'
  ...
  if [ "$(grep -Fxc -- "$observer_line" "$source_formula" || true)" != "1" ] ||
     ...
     [ "$(grep -Fxc -- "$host_preservation_comment" "$source_formula" || true)" != "1" ] ||
     [ "$(grep -Fxc -- "$host_preservation_comment" "$formula_snapshot" || true)" != "0" ] ||
     [ "$(grep -Fxc -- "$host_preservation_line" "$source_formula" || true)" != "1" ] ||
     [ "$(grep -Fxc -- "$host_preservation_line" "$formula_snapshot" || true)" != "0" ] ||
  ...
  cmp \
    <(tail -n +3 "$source_formula" |
      ...
      grep -Fvx -- "$host_preservation_comment" |
      grep -Fvx -- "$host_preservation_line" |
