...
run_isolation_gate() {
  ...
  runtime_commit="$(sed -n 's/^runtime_build_commit=//p' "$out" | tail -n1)"
  runtime_dirty="$(sed -n 's/^runtime_build_dirty=//p' "$out" | tail -n1)"
  runtime_required=0
  ...
  if [ "$status" -eq 0 ]; then
    HIDEOUT_RUNTIME_EVIDENCE_REQUIRED="$runtime_required" \
      ...
      HIDEOUT_RUNTIME_EVIDENCE_BUILD_COMMIT="$runtime_commit" \
      HIDEOUT_RUNTIME_EVIDENCE_BUILD_DIRTY="$runtime_dirty" \
