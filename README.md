...
The package includes the retained, digest-pinned `developer-standard` preview
runtime for macOS arm64. Inspect the exact image, source, size, inventory, and
SBOM status with `hideout runtime inspect developer-standard`. Selection is
explicit and does not change existing profiles; custom images and the existing
template path remain available without inheriting runtime readiness.

Install the pinned test agent as the non-root target, without `sudo` or a
host-global prefix:
