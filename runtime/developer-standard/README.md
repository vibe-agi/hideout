...
The builder bootstrap and guest package payloads use that same timestamped
snapshot rather than a rolling Debian mirror. The lock does not contain a
promoted output digest. Every local or CI output remains a development candidate
and cannot satisfy 031 preview evidence.
...
when that appliance cannot launch. It uses KVM when `/dev/kvm` is usable and
otherwise selects libguestfs `force_tcg`; TCG is slower but runs the same locked,
offline customization path. The selected backend and acceleration mode are
recorded in `build-provenance.json`. The retained workflow runs in the locked
privileged builder; local hosts still need an environment in which the appliance
preflight succeeds. The workflow installs the runner trust bundle at a fixed
path and configures APT to use that path explicitly; TLS verification remains
mandatory.
...
alone. A separate default-branch workflow validates successful candidate
artifacts before copying them into a private draft release; candidate branch
code retains only read authority. A draft remains unpromoted and cannot enter
the runtime catalog. `build.sh` emits the QCOW2, checksums,
