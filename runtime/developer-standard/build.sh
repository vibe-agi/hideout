...
if ! libguestfs-test-tool >/dev/null 2>&1; then
  runtime_die "libguestfs appliance preflight failed (acceleration=$RUNTIME_LIBGUESTFS_ACCELERATION); use the locked privileged Linux arm64 builder or a host whose appliance kernel is readable"
fi
...
jq -n \
  ...
  --arg libguestfsBackend "$LIBGUESTFS_BACKEND" \
  --arg libguestfsBackendSettings "$RUNTIME_LIBGUESTFS_BACKEND_SETTINGS" \
  --arg libguestfsAcceleration "$RUNTIME_LIBGUESTFS_ACCELERATION" \
  --argjson imageBytes "$(wc -c < "$image" | tr -d ' ')" \
  '{schema:$schema,revision:$revision,source:{commit:$commit,dirty:$dirty,sourceLockSHA256:$sourceLockSHA256},builder:{observedIdentity:$builderImage,expectedIdentity:$expectedBuilderImage,attestation:(if $builderImage == $expectedBuilderImage then "workflow-declared" else "native-unpinned" end),qemu:$qemu,libguestfs:$libguestfs,libguestfsBackend:$libguestfsBackend,libguestfsBackendSettings:$libguestfsBackendSettings,libguestfsAcceleration:$libguestfsAcceleration},output:{file:$image,sha256:$imageSHA256,bytes:$imageBytes},startedAt:$startedAt,completedAt:$completedAt,promoted:false}' \
