...
runtime_validate_source_tree() {
  ...
  jq -e '
    ...
    (.debianSnapshot.packageMirror == "https://deb.debian.org/debian") and
  ...
  if grep -Eqi '(^|[/_.-])(latest|current|daily)([/_.-]|$)|[?#]' < <(jq -r '.base.location, .node.location, .go.location, .debianSnapshot.releaseLocation, .debianSnapshot.packagesLocation' "$lock"); then
...
runtime_prepare_deb_bundle() {
  ...
  local work_dir="$5"
  local output_tar="$6"
  local mirror packages_sha apt_root packages_index deb_cache package_args
  ...
  [ "$mirror" = "https://deb.debian.org/debian" ] || runtime_die "unsupported Debian package mirror"
  packages_sha="$(runtime_sha256 "$packages_xz")"
  apt_root="$work_dir/apt"
  deb_cache="$cache_root/debs-$packages_sha"
  packages_index="$apt_root/state/lists/deb.debian.org_debian_dists_trixie_main_binary-arm64_Packages"
  rm -rf "$apt_root"
  mkdir -p "$apt_root/etc/apt" "$apt_root/state/lists/partial" "$deb_cache/partial"
  printf '%s\n' "deb [trusted=yes] $mirror trixie main" >"$apt_root/etc/apt/sources.list"
  : >"$apt_root/status"
