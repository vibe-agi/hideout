...
安装包已经包含 runtime catalog 与验证机制，但当前源码版本仍携带尚未晋级的空
catalog。只有当 `hideout runtime list` 显示已保留、digest 固定的 preview revision
后，才应加入 `--runtime developer-standard`。自定义镜像和现有 template 路径仍可
使用，但不会继承 runtime readiness。

preview runtime 晋级后，以非 root target 安装固定测试的真实 agent，不使用 `sudo`
或主机全局目录：
