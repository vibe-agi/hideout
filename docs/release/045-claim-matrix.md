...
T156's accepted clean-source performance aggregate passed local query/render,
daemon/TUI, five real-Chrome freshness samples, thirty measured real-Lima warm
attaches plus two warmups, paired reference work, observer CPU/RSS/event/loss
sampling, recovery, and real quota pressure at
`.artifacts/045/performance/run-20260801T070125Z-30370/summary.json`
(`sha256:9772762738aac4befe5bedbbfe75b9db7f4970e2d250819651e90177656b12a7`).
It binds clean commit `504514bbbf1feb1a00e05a1f5f6f52e78b5ed881`, a stable
1,754-file source manifest, and `candidateAcceptance=true`. Reference baseline
and observed medians were 585.274 ms and 618.137 ms: paired median overhead was
5.683%, and its exact one-sided 95% upper confidence bound was 6.44%, both
within the frozen 10% limit. Warm-attach p95 was 868.465 ms, browser freshness
p95 was 78.714 ms, observer CPU p95 was 38.8% of one guest vCPU, observer RSS
p95 was 72,990,720 bytes, and zero of 7,000 healthy events were dropped. Quota
pressure pruned oldest sealed history, exposed the retention gap, and stayed
within one active-segment allowance. All three exact claim receipts, 545
private artifacts, raw samples, byte counts, digests, modes, and 770 continuous
host-contention samples were independently rechecked. T156 is complete; T171
must repeat the aggregate after closure-document commits so the final package
and evidence bind the eventual exact HEAD.
...
| C04 | Authoritative ordering is monotonic per stream; duplicates/reconnects do not double count and unfillable gaps force reseed (FR-047–FR-048). | `go test ./internal/liveconsole ./internal/daemon ./internal/workloadobs/store -run 'Test(BrowserSSERequiresCurrentAuthoritativeSnapshotSequence\|BrowserSSEGapIsStickyReadOnlyUntilAuthoritativeReseed\|SealScopesBoundIndependentPerCPUSequencesWithoutTimelineOrdering)' -count=1` plus reducer sequence tests. | `scripts/gates/browser-console.sh` and T154 require an atomic snapshot-sequence subscription, SSE-only healthy updates, sticky gap state, and authoritative reseed. | `M045-C04`: open HTTP 200 before subscriber registration, accept a sequence gap, or double-apply a duplicate. `N045-C04`: event log skips a sequence while browser result says LIVE without snapshot reseed; judge must reject it. | `active-local-gate`; the deterministic flush barrier proves subscription precedes HTTP 200, and T154 proved zero duplicate count delta, sticky reseed on gaps, no false LIVE state, and no healthy polling. |
| C05 | Reusable history is exact-incarnation/session scoped; disposable history is lifecycle-scoped; quota/age pruning is ordered, bounded, and exposes a history gap (FR-049–FR-052, SC-010). | `go test ./internal/workloadobs/store ./internal/manager -run 'Test(QuotaPrunesOldestSealedAcrossOwnersAndBoundsOvershoot\|OwnerRetentionMaxAgePrunesExpiredSealedHistory\|ReusableOwnerQueriesRemainInsideSelectedSession\|DisposableActivityCleanupDeletesOnlyTerminalOwnerAndPreservesAudit)' -count=1` | `scripts/gates/workload-privacy-lima.sh` and T156 must bind quota/overshoot/pruning evidence to the exact owner. | `M045-C05`: prune newest/foreign owner or hide retention gap. `N045-C05`: quota artifact exceeds the bound, changes owner, or reports complete history after pruning; judge must reject it. | `active-real-backend-gate`; accepted T156 evidence proves stable query ownership, oldest-sealed pruning, no foreign-owner pruning, a visible history gap, and overshoot bounded by exactly one active segment. Exact final-package rebinding remains T171. |
| C06 | Attach, target-workload CPU/elapsed overhead, observer CPU/RSS, event/drop rate, storage overshoot, query/render latency, and UI freshness stay within frozen supported budgets (FR-068, SC-003, SC-011). Host-wide CPU is eligibility/invalidation evidence, not the overhead numerator. | `go test ./internal/workloadobs -run TestReleaseFrozenDefaultsRemainWiredAcrossPackages -count=1` locks the shared v1 defaults; existing bounded unit tests and `scripts/gates/browser-console.sh` enforce browser thresholds. | `HIDEOUT_PERFORMANCE_QUIET_HOST_CONFIRMED=1 scripts/gates/release-candidate-performance.sh` must evaluate raw child-process user/system CPU, paired elapsed samples, percentile and confidence-bound math, quota bounds, private host-state diagnostics, and exact candidate identity. Evidence-only follow-up must pass `scripts/release/revalidate-performance-evidence.sh`, which verifies every retained artifact and the exact non-product/non-measurement diff. | `M045-C06`: add measured delay/allocation, reduce a quota guard, or classify a product/measurement path as reusable. `N045-C06`: fixture forges CPU/elapsed percentile or confidence, omits resource samples, changes units, lacks quiet-host confirmation, changes the normalized measurement body, or exceeds one threshold while reporting passed; each judge must reject it. | `active-real-backend-gate`; the accepted 30-sample run reports 4.605% target-workload CPU median overhead with a 4.884% one-sided 95% upper bound and 4.474% paired elapsed median overhead with a 4.841% upper bound, with no reported loss. Exact-candidate reuse is allowed only through the content-addressed evidence-only receipt; otherwise T171 requires a fresh run. |
...
| RC04 | Stale plans, concurrent clients, observer/event loss, daemon crash, rollback, stop proof, and lifecycle deletion satisfy bounded safety and progress (FR-066, SC-004, SC-012). | The exact 17-config, 12-module, and 27-test inventory in `formal/inventory.json`, including production refinement, secret authority-reset, observer tail drain/forced-close accounting, and 16-point crash-matrix traces. | `scripts/gates/formal.sh` records all 142 inventoried invariant checks and 28 liveness properties in `.artifacts/045/formal/`; `scripts/gates/formal-verify.sh` independently rechecks the repository inventory, source/log digests, exact pass sets, worker count, and bounded heap. | `M045-RC04`: one named trace mutant per invariant/liveness boundary. `N045-RC04`: omit a required cfg/result, add a counterexample, or bind a stale model digest; all three formal-verifier fixtures are retained killed. | `active-local-formal`; bounded formal and dirty-aware real-Lima evidence pass with `candidateAcceptance=false`, while exact clean-candidate binding remains T163. |
...
- Accepted T156 performance evidence binds clean source commit `504514b`,
  thirty quiet-host samples, the exact confidence bound, continuous contention
  evidence, and every frozen performance/quota threshold. The final closure
  still must repeat it after documentation commits to bind the exact package.
- The dirty-source real-Lima, UI, and privacy aggregates plus clean-source
  performance aggregate pass,
