...
| `--rm` disposable-teardown phase 2: crash-orphan GC and the real-Lima gate lane. Phase 1 of the 2026-07-21 design is implemented with offline fault-injection coverage: resolution creates a dedicated disposable environment (unique `rm-<12hex>` name, `ModeDedicated`, `Active`, persistent `Record.Disposable`, `PreserveInstance=false`, pinned lima `InstanceName`, runtime-disk provenance precheck included; `SessionIsolationRequired` now holds, so the daemon streaming path accepts `--rm`), and teardown is proof-gated through `finishConcurrentRunEnvironment`: backend cleanup runs `limactl delete -f`, `confirmDisposableInstanceAbsent` requires two consecutive Absent inventory samples, and `disposeFinishedEnvironment` grades the outcome — proved → close the environment gateway and remove the record (session-cleanup errors still return); unproved, unobservable, or sibling-live → retain the record as `error` for reconcile/clean, never faking success. Target failure still disposes; `--rm`+`--env` stays rejected; `--rm`+`--ephemeral` stays orthogonal; `FinishRunEnvironment`'s dead removal path is gone. RunResult carries `environmentDisposition` (`removed`/`cleanup-required`) and the CLI names a retained record with a copyable `hideout clean`. The strengthened gate2 `--rm` lane (proved disposition, unchanged record count, no `rm-*` residue, unchanged limactl inventory) passed on real Lima in the 2026-07-22 integrated `GATE2_EXIT=0` run. Remaining before the claim: **(a)** provable crash-orphan GC — daemon orphan detection still reports-but-never-destroys; `Disposable=true` is user pre-authorization, but GC must prove no live owner, session terminated, exact instance absent, and gateway/runtime/journal cleared (model the GC protocol in TLA+ alongside, per `formal/` discipline); **(b)** coordinator journal convergence for removed records — measured, not hypothetical: the passing gate2 run still left one orphan `store/lifecycle/<id>/journal.json` for a removed environment (0 records, 1 journal at gate end), and with stable absence now failing stop closed, an auto-stop attempt against such residue blocks rather than converges; **(c)** one `--rm --ephemeral` combination lane in gate2. | Before `--rm` on lima is claimed as a supported daemon run and before daemon-restart behavior with disposable residue is relied on | 2026-07-21 design pass + codex review; 2026-07-22 phase-1 implementation, fault-injection matrix, and integrated `GATE2_EXIT=0` with the strengthened lane; orphan-journal residue observed in the same run |
