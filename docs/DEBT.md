...
| Homebrew tap parity portability is implemented but unproved in CI: the docs-smoke tap path now comes from `HIDEOUT_TAP_FORMULA` or a sibling `../homebrew-tap` checkout, records explicit not-run locally without one, and fails closed under `HIDEOUT_REQUIRE_TAP_PARITY=1`; the gate0 CI job checks out `vibe-agi/homebrew-tap` and sets both. A skipped parity check is not parity proof, so this entry is removed only after the first green CI run that executed the real comparison. | First green gate0 CI run on a runner without a pre-existing tap checkout | 038 acceptance; 2026-07-24 env-configurable parity + CI tap checkout |
