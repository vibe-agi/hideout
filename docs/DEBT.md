...
| `--ephemeral` / `--rm` on the lima backend fail under the daemon isolated-session model. `SelectRunEnvironment` maps them to a record-less `RunEnvironment` (`Active=false`), so `SessionIsolationRequired = runEnv.Active && backend=="lima"` (`run_apply.go`) is false and `RunWithStreams` — the daemon's only streaming path (`backend/lima/session_stream.go`) — rejects the run with `lima daemon session requires the isolated session supervisor`. But `--ephemeral` is documented as *session-local identity state* (`app.go`), not a record-less environment; the correct fix is for ephemeral to resolve the shared default VM (48af97e) with session-local identity (via `RunIdentityDir`), so the isolated ready proof (`backend.go` `SessionReadyProof.Validate`) has the `EnvironmentID`+`InstanceName`+`BootID` it requires. Proven feature-level scope (2026-07-21): (1) env-resolution `Create` semantics — `Create:false` yields a transient `env_new` whose runtime dir does not exist, so the run path `lstat`-fails; ephemeral runs likely must force a persisted shared env; (2) a resolved auto-named env emits `environmentName`, which `run-result.schema.json` forbids (a contract change); (3) ≥4 tests encode the old "record-less by contract" behavior and would need updating; (4) ephemeral would newly attach to the shared-env lifecycle (transition lock, lifecycle registration, final-session-stop) — composition must be verified. `--rm` (dispose a *dedicated* env after the run) is a related but distinct sub-case. The in-process `--rm` app-test passes only because it uses the non-streaming `backend.Run` path (`opts.Streams==nil`), which the daemon never takes. | Before `--ephemeral` / `--rm` on lima are claimed or tested as supported daemon runs — the gate2 "ephemeral identity smoke" lane is the live trigger and fails today, blocking a clean post-live-read `GATE2_EXIT=0` (and therefore the named-environment lane that gate-verifies the lazy-reuse network change) | 2026-07-21 gate2 post-live-read revival; the `SessionIsolationRequired` coupling was introduced by c29365b "support concurrent run sessions" |
