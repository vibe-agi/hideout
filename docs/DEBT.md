...
| `trusted-host-ide` is unusable for one-shot commands: the trusted `host-app.open-resource` decision is bound to the live run, but `code .` triggers the open and exits before an operator can claim/approve, so the decision goes stale. 030 design intent is a per-profile+workspace persisted grant a one-shot open can reuse; implementation landed as per-live-run. Path B (persistent grant in `runProjectionGrantChecker`) proven by throwaway spike 2026-07-20; queued as 039 | Before trusted-host-ide is promoted as the native-editor path (README/first-run currently steer one-shot `code .` to safe mode) | 2026-07-20 trusted-ide walkthrough |
| Two parallel projection grant checkers: production wires `runProjectionGrantChecker` (`run_dataplane.go:178`); `decisionIdeGrantChecker` (`hostcap_projection.go:335`) has only test callers. Dead/test-only parallel implementation | 039 implementation should delete or explain the twin | 2026-07-20 039 spike |
