...
| Internal type and file names still read `TrustedIDEGrant*` / `trusted_ide_grant.go` / `trusted-ide-grant.schema.json` / spec dir `039-trusted-ide-grant` after 039 generalized the user-visible surface to `host-app` (`allow host-app <command>`, `profile host-app-mode`, audit `mode=trusted-host-app`). The split is intentional (no user-visible leak) but the historical names remain | Opportunistic rename next time these files are substantially touched | 2026-07-20 039 generalization |
