...
`678f37237f44`. Public GA still requires this evidence to be produced for the
final release artifact and explicit release-specific signoff.

Latest local release-candidate evidence:

- manifest:
  `.hideout-release-evidence/release-dogfood-20260705T003128Z-678f37237f44/manifest.json`
- status: passed
- command: `scripts/test-phase1.sh --release-candidate`
- commit: `678f37237f44`, dirty: false
- host: Darwin 25.4.0 arm64, macOS 26.4.1
- release artifact: `hideout-darwin-arm64-678f37237f44.tar.gz`; SHA-256 and
...
| TUI smoke surface | `hideout tui` renders a capped terminal dashboard from Manager overview and redacted audit data, including local refresh time, init next steps, capability summary, network risk, reusable environments with lifecycle command hints, sessions with audit/cleanup command hints, recent denied audit, and recent audit. `--watch` refreshes locally without starting WebUI or minting a UI token. |
| WebUI smoke surface | Embedded local WebUI shows overview/audit/resource summaries, local refresh time, UI token expiry time, capped reusable environment and session panels with audit/cleanup command hints, init next steps, network risk, denied-audit counts, basic audit filtering, generic tool setup with next-step rendering, controlled run plan/apply, and reusable environment stop/clean plan/apply through Manager API. It remains a lightweight smoke/operations surface, not the final product UI. |
