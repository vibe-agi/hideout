...
`8053c59e57c3`. Public GA still requires this evidence to be produced for the
final release artifact and explicit release-specific signoff.

Latest local release-candidate evidence:

- manifest:
  `.hideout-release-evidence/release-dogfood-20260705T011048Z-8053c59e57c3/manifest.json`
- status: passed
- command: `scripts/test-phase1.sh --release-candidate`
- commit: `8053c59e57c3`, dirty: false
- host: Darwin 25.4.0 arm64, macOS 26.4.1
- release artifact: `hideout-darwin-arm64-8053c59e57c3.tar.gz`; SHA-256 and
...
| WebUI smoke surface | Embedded local WebUI shows overview/audit/resource summaries, local refresh time, UI token expiry time, optional profile scope filtering, capped reusable environment and session panels with audit/cleanup command hints, init next steps, network risk, denied-audit counts, basic audit filtering, generic tool setup with next-step rendering, controlled run plan/apply, and reusable environment stop/clean plan/apply through Manager API. It remains a lightweight smoke/operations surface, not the final product UI. |
