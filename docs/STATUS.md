...
`31efb9b0d82e`. Public GA still requires repeatable evidence for release
artifacts, packaging/distribution closure, and release-specific signoff.

Latest local release-candidate evidence:

- manifest:
  `.hideout-release-evidence/release-dogfood-20260704T191407Z-31efb9b0d82e/manifest.json`
- status: passed
- command: `scripts/test-phase1.sh --release-candidate`
- commit: `31efb9b0d82e`, dirty: false
...
| TUI smoke surface | `hideout tui` renders a terminal dashboard from Manager overview and redacted audit data. `--watch` refreshes locally without starting WebUI or minting a UI token. |
| WebUI smoke surface | Embedded local WebUI shows overview/audit/resource summaries and exposes generic tool setup plus controlled run plan/apply through Manager API. It remains a lightweight smoke/operations surface, not the final product UI. |
