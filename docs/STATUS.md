...
| `hideoutd` daemon | Implemented (local control plane). `hideout daemon start\|status\|stop` runs one per-user daemon per store, serving the existing typed Manager API by mounting the same handler over a store-rooted, guest-unreachable Unix socket (parity by construction; daemon status/event endpoints live outside `/api/v1/`). Every request is operator-token authenticated; unauthenticated refusals are recorded in a persistent, session-unbound daemon-local audit log without token material. It emits a live, redacted event stream (operation lifecycle + session audit tail; no durable log, one-`overview` seed, streams terminate on credential expiry), serves the WebUI over a tokened loopback transport whose panels refresh on that stream via `EventSource` (the TUI via `daemon.SubscribeEvents`; both event-triggered re-fetch with no polling timer, falling back to prior behavior with no daemon; payload-driven panel updates and end-to-end UI verification are a deferred follow-on), accepts existing typed environment stop/clean apply as background work through `POST /daemon/background` with queryable status and bounded fail-closed shutdown. After restart it fails closed for live resources it cannot prove it owns (report + audit, never re-adopt or destroy). Daemon absence leaves every embedded surface unchanged. Prompt channels are out of scope. |
