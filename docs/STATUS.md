...
| WebUI smoke surface | Embedded local WebUI shows overview/audit/resource summaries, local refresh time, capped reusable environment and session panels with audit/cleanup command hints, init next steps, network risk, denied-audit counts, basic audit filtering, generic tool setup with next-step rendering, controlled run plan/apply, and reusable environment stop/clean plan/apply through Manager API. It remains a lightweight smoke/operations surface, not the final product UI. |
