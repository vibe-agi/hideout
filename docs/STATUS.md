...
`7df9b88e3a4e`. Public GA still requires this evidence to be produced for the
final release artifact and explicit release-specific signoff.

Latest local release-candidate evidence:

- manifest:
  `.hideout-release-evidence/release-dogfood-20260705T062339Z-7df9b88e3a4e/manifest.json`
- status: passed
- command: `scripts/test-phase1.sh --release-candidate`
- commit: `7df9b88e3a4e`, dirty: false
- host: Darwin 25.4.0 arm64, macOS 26.4.1
- release artifact: `hideout-darwin-arm64-7df9b88e3a4e.tar.gz`; SHA-256
  `ee78443d627d1c90a6a6f7e6aa40a9fdddb2b0e751269f3cec66bfef8bb59edb`;
...
| TUI smoke surface | `hideout tui` renders a capped persistent terminal dashboard from Manager overview and redacted audit data, including local refresh time, optional profile filtering, init next steps, capability summary, per-profile env, tool, command-proxy, and HostFS rule counts with CLI setup hints, network risk, reusable environments with lifecycle command hints, sessions with audit/cleanup command hints, recent denied audit, and recent audit. It refreshes locally without starting WebUI or minting a UI token. `--once` is the script/package-smoke snapshot mode; full-screen keyboard navigation and selectable session/event panes are design-ready. |
