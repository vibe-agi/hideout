...
| TUI smoke surface | `hideout tui` renders a terminal dashboard from Manager overview and redacted audit data, including capability summary, network risk, sessions, recent denied audit, and recent audit. `--watch` refreshes locally without starting WebUI or minting a UI token. |
| WebUI smoke surface | Embedded local WebUI shows overview/audit/resource summaries, network risk, denied-audit counts through Manager API filtering, generic tool setup with next-step rendering, and controlled run plan/apply through Manager API. It remains a lightweight smoke/operations surface, not the final product UI. |
